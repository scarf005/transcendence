...
export class MatchService {
  ...
  removeFromQueue(player: Socket) {
    console.log('before remove', this.quickQueue.map(elem => elem.uid))
    this.quickQueue.splice(this.quickQueue.findIndex(p => p.socket.id === player.id), 1)
    this.rankedQueue.splice(this.rankedQueue.findIndex(p => p.socket.id === player.id), 1)
    console.log('after remove', this.quickQueue.map(elem => elem.uid))
  }
}
